...
export function MobileNav() {
  ...
  const overlay = (
    <div
      className={cn(
        "mobile-nav-overlay fixed inset-0 transition-opacity duration-300 md:hidden",
        open ? "pointer-events-auto opacity-100" : "pointer-events-none opacity-0",
      )}
      aria-hidden={!open}
    >
      <div className="mobile-nav-backdrop absolute inset-0" onClick={() => setOpen(false)} />
      <nav
        className={cn(
          "mobile-nav-panel absolute right-0 top-0 flex h-full w-[min(100%,20rem)] flex-col border-l border-app-border p-6 transition-transform duration-300 ease-out",
