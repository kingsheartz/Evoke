...
export default function EditPackagePage() {
  ...
  if (!pkg) return <p className="text-sm text-zinc-500">Loading...</p>;

  return (
    <div className="space-y-8">
      <div>
        <Link href="/admin/tours/packages" className="text-sm text-zinc-500">← Back</Link>
        <h1 className="mt-2 text-3xl font-bold">Edit Package</h1>
      </div>
      <Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {days.map((day) => (
            <div key={day.id} className="flex items-start justify-between rounded-lg border border-zinc-100 p-4">
              <div>
                <p className="font-medium">Day {day.day_number}: {day.title}</p>
                <p className="mt-1 text-sm text-zinc-500">{day.description}</p>
              </div>
              <Button variant="ghost" size="sm" onClick={() => removeDay(day.id)}><Trash2 className="h-4 w-4 text-red-500" /></Button>
            </div>
          ))}
          <div className="grid gap-3 rounded-lg border border-dashed border-zinc-200 p-4 md:grid-cols-3">
